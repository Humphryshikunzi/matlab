...
% Copyight 2014-2016 The Mathworks, Inc.
            ...
            if ~exist(FGHL20Directory,'dir')
                ...
                if statusFG
                    if messageFG.UserWrite
                        ...
                        if ~statusCopy
                            warning(message('aeroblks:asbhl20:copyFail',messageCopy));
                        end
                    end
                else
                    statusCopy = 0;
                    warning(message('aeroblks:asbhl20:FGAircraftDirectoryNotFound',messageFG));
                end
            end
            % Proceed to create the batch file in the current directory and launch
            % FlightGear.
            if statusCopy
                % Obtain FlightGear version from the model
                maskVals = get_param([mdl '/Viewer and Feedback/FlightGear/Generate Run Script'],'MaskValues');
                ...
                if messagePWD.UserWrite
                    ...
                    if ~strcmp(pwd,workDir)
                        movefile(fullfile(pwd,maskVals{1}),workDir);
                    end
                    % Perform system call for batch file that starts up FlightGear
                    if exist(fullfile(workDir,maskVals{1}),'file')
                        if ispc
                            statusSys = system([fullfile(workDir,maskVals{1}) '&']);
                            % Set Variant to FlightGear
                            evalin('base','Variants = setfield(Variants,''FlightGear'',1);');
                        else
                            % Get current directory
                            oldDir = pwd;
                            % Go to work directory and execute batch file
                            cd(workDir);
                            statusSys = system(['./' maskVals{1} '&']);
                        ...
                        if statusSys~=0
                            error(message('aeroblks:asbhl20:systemFail'));
                        end
                    else
                        error(message('aeroblks:asbhl20:batchFail'));
                    end
                end
            end
        else
            % Set Variant to Previously Saved Data
            evalin('base','Variants = setfield(Variants,''FlightGear'',0);');
            error(message('aeroblks:asbhl20:baseFail'));
        end
    case 'Check'
        ...
        if exist(FGExecFile,'file')
            ...
            if strcmp(sourceCall,'button')
                ...
                msgbox(getString(message('aeroblks:asbhl20:FGDirectoryFound')),...
                getString(message('aeroblks:asbhl20:FGBaseFoundTitle')));
            end
           ...
           if ~strcmp(FGBase,FGBaseCheck)
            ...
            if strcmp(sourceCall,'button')
                if exist(FGBaseDirectory,'dir')
                    ...
                    'FlightGearVersion');
                    errordlg(getString(message('aeroblks:asbhl20:FGFail',FGVersion,FGVersion)),...
                    getString(message('aeroblks:asbhl20:FGExecutableNotFoundTitle')));
                else
                    % Throw error dialog if the check is performed using the button
                    errordlg(getString(message('aeroblks:asbhl20:baseFail')),...
                    getString(message('aeroblks:asbhl20:FGBaseNotFoundTitle')));
                end
                % Set Variant to Previously Saved Data
                evalin('base','Variants = setfield(Variants,''FlightGear'',0);');
            end
...
switch computer
    ...
    case 'GLNXA64'
        FGExecFile = fullfile(FGBaseDirectory,'fgfs.sh');
        FGAircraftDirectory = fullfile(FGBaseDirectory,'data','Aircraft');
end
